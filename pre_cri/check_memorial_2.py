...
def gerar_memorial_apartamento(row):
    ...
    memorial = (
        ...
        f"Fração ideal de solo e partes comuns no subcondominio: {fracao_ideal_unidade_subcondominio:.8f}; "
...
def gerar_memorial_vaga(row):
    ...
    if deposito_vinculado:
        memorial += (
            f"área de depósito nº {deposito_numero} de {area_vinculada_outras:.2f} metros quadrados, "
        )
    
    memorial += (
        f"e a área comum de {area_comum:.2f} metros quadrados. "
        f"Fração ideal de solo e partes comuns no subcondomínio: {fracao_ideal_unidade_subcondominio:.8f}; "
